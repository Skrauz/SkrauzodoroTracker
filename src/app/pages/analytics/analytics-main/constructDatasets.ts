    ...
    timespans.forEach((timespan) => {
      ...
      labels?.forEach((label, index) => {
        if (label == formatDateEnGB(timespanEndDate)) {
          let dataset: Dataset = {
            data: [],
            label: timespan.name ? timespan.name : "Unnamed Task",
          ...
          dataset.data[index] = getTimeDifference(timespan.endTime, timespan.startTime);
...
function getTimeDifference(date1: Date, date2: Date): number {
  let difference = Math.abs(new Date(date1).valueOf() - new Date(date2).valueOf());
